...
def mock_vector_store():
    """A `VectorStore`-shaped MagicMock with empty-but-valid defaults.

    Override return values per-test, e.g.:
        mock_vector_store.get_existing_course_titles.return_value = ["X"]
    """
    store = MagicMock()
    # A SearchResults-shaped default: no docs, no error.
    ...
    store.search.return_value = empty_results
